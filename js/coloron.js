class Game {
    ...
    generateTweet() {
        ...
        let text = '我在卡羅頓得到 ' + this.score + ' 分！您能打破我的紀錄嗎？';
        let tweetUrl = 'https://twitter.com/intent/tweet?url=https://github.com/henryleeworld/js-coloron&text=' + text + '&via=李亨利&hashtags=卡羅頓';
        window.open(encodeURI(tweetUrl), "TweetWindow", "width=600px,height=300px,top=" + top + ",left=" + left);
    }
    ...
    showGrade(score) {
        if (score > 30) return "傳奇？";
        else
        if (score > 25) return "做得超棒！";
        else
        if (score > 20) return "超好！";
        else
        if (score > 15) return "好棒！";
        else
        if (score > 13) return "很好！";
        else
        if (score > 10) return "做得好！";
        else
        if (score > 5) return "真的嗎？";
        else
            return "菜鳥...";
    }
